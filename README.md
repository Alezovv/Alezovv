...
*  Специализируюсь на высоконагруженных системах и автономных AI-агентах.
...
### DocumentRegistry
*Локальное desktop-приложение для автоматической регистрации и обработки входящих документов.*
* **Document Processing Pipeline:** Реализовал полный offline-пайплайн `PDF → text extraction/OCR → semantic parsing → SQLite`.
* **OCR:** Интегрировал локальный **PaddleOCR** через ONNX-модели для обработки сканированных документов; для PDF с текстовым слоем OCR не запускается.
* **Local LLM:** Семантическое извлечение структурированных данных выполняется локально через **llama.cpp** и GGUF-модель, без облачных API.
* **Portable:** Собрал полностью автономную Windows-версию через **PyInstaller**, включающую Python runtime, OCR-модели, LLM runtime и модели.
* **Data:** Локальное хранение документов и метаданных в файловой системе и **SQLite**.
* **Стек:** Python 3.12, PyMuPDF, PaddleOCR, PaddleX, ONNX Runtime, llama.cpp, GGUF, SQLite, PyInstaller.
